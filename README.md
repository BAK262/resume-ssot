...
```text
工作区：D:\career\。请根据我的经历档案扩面检索适配岗位（官网、Boss、猎聘、智联等均可）。
硬过滤：[城市，例如北京]；入职窗口：[例如 2027 夏/秋]。
只收录能打开 JD 详情页的岗位；写进可投岗位表。选定后再做投递版简历；无官网的岗按平台通道投递。
```
...
| 可投岗位表 | JD 详情链可打开（官网或第三方均可）；可改类别（待确认/待投递/候补/归档）。**第三方**：入库待确认 → 平台打招呼 → 待投递 → 发简历后进进度表；官网直投定稿后可先入进度表待投递 |
