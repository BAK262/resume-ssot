...
| 阶段 | 动作 |
| --- | --- |
| 扩岗入库 | `category` 默认 `pending`；**不**自动打招呼 |
| 用户确认待投 | 改为 `ready` → 出投递版（[resume.md](resume.md) Mode · jd） |
| 无官网、仅第三方 | **平台沟通/打招呼/在线投递即主通道**，不是备胎；按 `why` 执行 |
| 确认已投递 | [offer.md](offer.md)：进度表记「Boss 已打招呼 / 猎聘已投递 / 邮件已发」等；移出可投表 |

平台注意：Boss 有日沟通上限；待投递后再个性化沟通（可复用 [resume.md](resume.md) Mode · pitch 开场）；控制频率，避免批量泛招呼触发风控。
