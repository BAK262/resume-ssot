...
每个 agent 只负责一家：打开招聘站 → brief 关键词检索 → 过硬过滤 → 打开 JD → 对照 `ssot_summary` 初匹配。

**两条入口**

| 入口 | 做法 |
| --- | --- |
| 官网扩岗（默认） | 企业 `portal_url` 实搜 |
| 用户/第三方直链 | 打开用户给的 Boss/猎聘/智联/邮件链 → **入库前补官网回链**（见下） |

**官网回链**（第三方岗入库前必做）

1. 找官源：官网招聘页、校招门户、官方邮箱/公告、猎聘/智联**企业直招**帖（署名本公司 HR）
2. 猎头匿名但有猎聘**同文案**帖 → 可作交叉核验，**勿**当雇主官网
3. 仅有第三方、无官源 → 记「无官网招聘页；投递：Boss 直聊 / 邮件 xxx」
...
- `why` = 匹配理由 1–2 句（锚经历档案）+ **第三方岗**在末尾补薪酬/官源/投递方式（有则记，无则略）
