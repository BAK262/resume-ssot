...
1. 从邮件 / 官网 / 第三方平台截图 / 用户口述提取：日期、公司、岗位、状态、下一步、备注；有则填 `portalUrl`（官网个人中心，有则记）/ `jdUrl`（JD 详情，官网或第三方链均可）/ 内推。
...
| 用户确认已投递 / 邮件回执 | `已投递`（或证据对应状态） | 证据一句；第三方岗注明通道（如「Boss 已打招呼」「猎聘已投递」「邮件已发」） |
