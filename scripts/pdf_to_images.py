...
def _render_magick(
    *,
    ...
    for page_no in pages:
        idx = page_no - 1
        out_path = out_dir / f"{prefix}{page_no:0{pad}d}.{fmt}"
        _run([exe, "-density", str(dpi), f"{in_pdf}[{idx}]", str(out_path)])


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Split a PDF into per-page images for visual review (resume-ssot)."
    )
    ...
    parser.add_argument("--prefix", default="page_", help='Output filename prefix (default: "page_")')
    parser.add_argument(
        "--engine",
    ...
    if engine == "none":
        raise RuntimeError(
            ...
            "- Or install Poppler (pdftoppm) / ImageMagick (magick) on PATH."
        )
...
if __name__ == "__main__":
    try:
        ...
        print(f"[resume-ssot pdf_to_images.py] ERROR: {exc}", file=sys.stderr)
        raise
